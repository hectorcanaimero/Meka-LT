...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    mekaLTAuthUserSubject.add(
      MekaLTAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<MekaLTAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  MekaLTAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = MekaLTAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    mekaLTAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? UsersStruct.fromSerializableMap(
